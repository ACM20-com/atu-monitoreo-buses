...
public class ServidorATU {
            ...
            while ((linea = br.readLine()) != null) {
                ...
                if (partes.length == 3 || partes.length == 4) {
                    try {
                        ...
                        // Corredor es opcional (compatibilidad con clientes viejos de 3 campos)
                        String corredor = partes.length == 4 ? partes[3].trim().toUpperCase() : "ROJO";
                        guardarUbicacion(idBus, latitud, longitud, corredor);
                        mensajesRecibidos++;
                    ...
                    pw.println("ERROR|SE_ESPERAN_3_O_4_CAMPOS");
                }
            }
        } catch (Exception e) {
    ...
    static void guardarUbicacion(String idBus, double lat, double lon, String corredor) {
        String sqlInsert = "INSERT INTO ubicaciones (id_bus, latitud, longitud, corredor) VALUES (?, ?, ?, ?)";
        try (Connection conn = DriverManager.getConnection(DB_URL, DB_USER, DB_PASS);
            ...
            ps.executeUpdate();
            System.out.println("[DB] Guardado: Bus=" + idBus + " lat=" + lat + " lon=" + lon + " corredor=" + corredor);
    ...
    static String obtenerUltimasUbicaciones() {
        // Subconsulta: para cada bus, trae la fila completa (incluido corredor) de su timestamp mas reciente
        String sql = "SELECT u.id_bus, u.latitud, u.longitud, u.corredor, u.timestamp as ultima " +
                     "FROM ubicaciones u " +
                     "INNER JOIN (SELECT id_bus, MAX(timestamp) as max_ts FROM ubicaciones GROUP BY id_bus) m " +
                     "ON u.id_bus = m.id_bus AND u.timestamp = m.max_ts " +
        ...
        try (Connection conn = DriverManager.getConnection(DB_URL, DB_USER, DB_PASS);
            ...
            while (rs.next()) {
                ...
                sb.append(String.format(Locale.US,
                    "{\"id_bus\":\"%s\",\"latitud\":%f,\"longitud\":%f,\"corredor\":\"%s\",\"timestamp\":\"%s\"}",
                    rs.getString("id_bus"),
